...
首次使用 Windows 时，双击根目录的 `setup_env.cmd` 可一键安装后端虚拟环境、Python 依赖和前端 npm 依赖；已有 `backend/.env` 会被保留。需要真实 PaddleOCR 运行环境时可在终端执行 `setup_env.cmd --with-ocr`，默认安装不会下载体积较大的 OCR 依赖。
